...
func calchange(change int) {
	wr := bufio.NewWriter(os.Stdout)
	defer wr.Flush()
	changearr := [4]int{25, 10, 5, 1}
	n_changearr := [4]int{0, 0, 0, 0}
	temp := change
	for i := 0; i < 4; i++ {
		n_changearr[i] = temp / changearr[i]
		temp %= changearr[i]
	}
	wr.WriteString(strconv.Itoa(n_changearr[0]) + " " + strconv.Itoa(n_changearr[1]) + " " + strconv.Itoa(n_changearr[2]) + " " + strconv.Itoa(n_changearr[3]) + "\n")
