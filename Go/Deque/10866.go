...
func createDeque() *Deque { //struct 생성 메소드
	queue := Deque{}
	queue._size = 0
	queue._back = nil
	queue._front = nil
...
func (dq *Deque) push_front(newInt int) { //포인터로 접근
	newNode := Node{}
	newNode.data = newInt
	if dq.size() == 0 {
		...
		newNode.prev = dq._front
		dq._front.next = &newNode
		dq._front = &newNode
	}
	dq._size++
}

func (dq *Deque) push_back(newInt int) { //포인터로 접근
	newNode := Node{}
	newNode.data = newInt
	if dq.size() == 0 {
		newNode.next = &newNode
		...
		newNode.next = dq._back
		dq._back.prev = &newNode
		dq._back = &newNode
	}
	dq._size++
}

func (dq *Deque) pop_front() int {
	if dq._size == 0 {
		return -1
	}
	data := dq._front.data
	dq._front = dq._front.prev
	dq._size--
	return data
}

func (dq *Deque) pop_back() int {
	if dq._size == 0 {
		return -1
	}
	data := dq._back.data
	dq._back = dq._back.next
	dq._size--
	return data
}
	...
	*/
	...
	for i := 0; i < n; i++ {
		...
		if len(x) > 1 {
			...
			} else if x[0] == "push_back" {
				queue.push_back(tmp)
			}

		} else {
			if x[0] == "pop_front" {
				wr.WriteString(strconv.Itoa(queue.pop_front()) + "\n")
			} else if x[0] == "pop_back" {
				wr.WriteString(strconv.Itoa(queue.pop_back()) + "\n")
			} else if x[0] == "front" {
